...
void
mpegts_packetizer_apply_skew_correction (MpegTSPacketizer2 * packetizer,
    GstClockTime pts, GstBuffer * buffer)
{
  GstClockTime time, btime = GST_BUFFER_TIMESTAMP (buffer);

  if (GST_CLOCK_TIME_IS_VALID (packetizer->base_time) &&
      GST_CLOCK_TIME_IS_VALID (packetizer->base_pcrtime) &&
      GST_CLOCK_TIME_IS_VALID (pts)) {
    time = (pts - packetizer->base_pcrtime) + packetizer->base_time;

    /* skew can be negative and we don't want to make invalid timestamps */
    if (packetizer->skew < 0 && time < -packetizer->skew)
      time = 0;
    else
      time += packetizer->skew;
    GST_BUFFER_TIMESTAMP (buffer) = time;
    GST_DEBUG_OBJECT (buffer, "Applying skew correcton: %" GST_TIME_FORMAT,
        GST_TIME_ARGS (ABS (time - btime)));
  }
}
 ...
 *
 * This is the difference between the PCR timestamp in the first received packet
    ...
    GstClockTime time)
{
   ...
   * should still apply a timestamp based on the PCR timestamp and base_time */
  ...
  if (G_UNLIKELY (packetizer->window_filling)) {
    ...
    if (G_UNLIKELY (send_diff >= MAX_TIME || pos >= MAX_WINDOW)) {
      ...
      packetizer->skew = (perc * packetizer->window_min +
          ((10000 - perc) * packetizer->skew)) / 10000;
...
no_skew:
  ...
  if (packetizer->base_time != -1) {
    ...
    /* We simply use the time without applying any skew compensation */
    out_time = time;
  }
