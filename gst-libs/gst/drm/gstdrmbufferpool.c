...
//      dump_list (self, self->head);
...
#ifdef DEBUG
      {
        ...
        self->nbbufs++;
        GST_DEBUG_OBJECT (self, "Creating new buffer %i", self->nbbufs);
        self->buffs = g_list_prepend (self->buffs, buf);
        for (tmp = self->buffs; tmp; tmp = tmp->next)
          GST_DEBUG_OBJECT (tmp->data, "");
      }
#endif /* DEBUG */
...
gst_drm_buffer_finalize (GstDRMBuffer * self)
{
...
#ifdef DEBUG
  {
    ...
    for (tmp = pool->buffs; tmp; tmp = tmp->next)
      GST_DEBUG_OBJECT (pool, "%p refs %i", tmp->data, GST_MINI_OBJECT (tmp->data)->refcount);
