 ...
 * should try to use. It takes into account the number of cores avalaible.
 *
...
static void
emit_error (GstTranscoder * self, GError * err)
{
  if (g_signal_handler_find (self, G_SIGNAL_MATCH_ID,
...
static void
emit_warning (GstTranscoder * self, GError * err)
{
...
error_cb (G_GNUC_UNUSED GstBus * bus, GstMessage * msg, gpointer user_data)
{
  ...
  transcoder_err =
      ...
      full_message);
  emit_error (self, transcoder_err);
...
warning_cb (G_GNUC_UNUSED GstBus * bus, GstMessage * msg, gpointer user_data)
{
  ...
  transcoder_err =
      g_error_new_literal (GST_TRANSCODER_ERROR, GST_TRANSCODER_ERROR_FAILED,
      full_message);
  emit_warning (self, transcoder_err);
...
gst_transcoder_main (gpointer data)
{
  ...
  if (self->transcodebin) {
    gst_element_set_state (self->transcodebin, GST_STATE_NULL);
    GST_ERROR_OBJECT (self, "Num refs: %d",
        G_OBJECT (self->transcodebin)->ref_count);
...
static void
_error_cb (GstTranscoder * self, GError * error, RunSyncData * data)
{
  g_mutex_lock (&data->m);
  data->done = TRUE;
 ...
 * @self: The GstTranscoder to run
 *
 * Run the transcoder task asynchonously. You should connect
...
static void
    ...
    (GstTranscoderSignalDispatcher * iface,
